...
<html>
	<head>
		<meta charset="UTF-8">
		<title>All available flights Page</title>
	</head>
	<body>
		<%
		try{
			...
			if(sort.equals("departTime")){
				query = "SELECT reservationId, departTime from reservationList, flightoperation "
						+ "where flight1 = flightnum order by " + sort;
			}
 			else if(sort.equals("flightDuration")) {
				query = "SELECT reservationId from reservationList order by totalFlightDuration";
			} 
			else {
				query = "SELECT reservationId from reservationList ORDER BY " + sort;
			}
		...
		<form action = "CustomerRegister.jsp" method = "post">
			<b><br>Enter the trip number to register to that flight:</b>
